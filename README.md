...
Part of **[Agent Kaizen](https://github.com/DirectorGunner/agent-kaizen)** — continuously improving how AI agents reason, act, verify, and operate inside project environments. This repository is the `git` skill: a reusable, trigger-rich task handbook that an AI coding agent (OpenAI Codex, Claude Code) loads on demand when a task matches its triggers.
